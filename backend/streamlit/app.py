...
if uploaded_file is not None:
    ...
    chunk_size = st.slider('Chunk Size', min_value=1, max_value=5000, value=500, key='chunk_size')
    chunk_overlap = st.slider('Chunk Overlap', min_value=0, max_value=1000, value=100, key='chunk_overlap')
    splitter_type = st.selectbox('Text Splitter Type', list(Splitter), format_func=lambda x: x.value)
    ...
    if 'chunks' not in st.session_state or 'cache_key' not in st.session_state or st.session_state['cache_key'] != cache_key:
        ...
        chunks, references = split_text(text_data,splitter=splitter_type, splitter_options=SplitterOptions(chunk_size, chunk_overlap), meta_data={"file_name": uploaded_file.name})
    ...
    df_chunks = pd.DataFrame(chunks, columns=['page_content', "metadata", "type"])

    # Convert the 'metadata' column to a string
    df_chunks['metadata'] = df_chunks['metadata'].astype(str)
